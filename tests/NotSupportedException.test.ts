...
import { NotSupportedException } from "../mod.ts";
...
import {
  ...
  initCases,
  messageCases,
} from "./_fixtures/NotSupportedException.cases.ts";
//  #endregion

//  #region tests
@TestSuite("NotSupportedException")
class NotSupportedExceptionTest {
  ...
  @TestCase(...initCases)
  @TrackFailedCase
  public testWithInit([data, exMsg]: [NotSupportedExceptionInit, string]) {
  ...
  @Test("(message, init)")
  @TestCase(...allCases)
  public testAll([data, exMsg]: [NotSupportedExceptionInit, string]) {
    const dataEncoded = encodeURIComponent(JSON.stringify(data));
...
Testing(NotSupportedExceptionTest);
